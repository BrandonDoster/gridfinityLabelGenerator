...
import { fitText, resolveBoxes } from "./layout";
import { LABEL_FONT, measureLabelInk } from "./textMetrics";
...
function textEl(text: string, box: Box): string {
  const { fontSize, baselineY } = fitText(text, box, measureLabelInk);
  return (
    `<text x="${box.x + box.w / 2}" y="${baselineY}" text-anchor="middle" ` +
    `font-size="${fontSize}" fill="${INK}" font-weight="bold" ` +
...
function buildLabelFaceSvg(label: LabelInput): { svg: string; pxW: number; pxH: number } {
  ...
  const slots = resolveBoxes(label, { iconBox: ICON_BOX, line1Box: LINE1_BOX, line2Box: LINE2_BOX });
  const line1 = adjustBox(slots.line1, placement.line1);
  const line2 = adjustBox(slots.line2, placement.line2);

  const body: string[] = [];
  body.push(iconEls(label, adjustBox(ICON_BOX, placement.icon)));
  if (label.line1) body.push(textEl(label.line1, line1));
  if (label.line2Svg) body.push(imageEl(label.line2Svg, label.line2ViewBox ?? SCREW_SVG_VIEWBOX, line2));
  else if (label.line2) body.push(textEl(label.line2, line2));
